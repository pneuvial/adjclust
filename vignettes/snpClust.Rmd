...
These data are drawn from the International HapMap Project and concern `r p` SNPs[^1] over a 1Mb region of chromosome 22 in sample of `r nSamples` Europeans.

We can compute and display the LD between these SNPs.

[^1]: We have dropped SNP `r colnames(ceph.1mb)[316]` because it produced a missing value due to the lack of genetic diversity in the considered sample.
...
We can now apply `snpClust` to this LD matrix (of class `r class(ld.ceph)` ie Matrix::dgCMatrix):
...
```{r, warning = FALSE}
gmat <- as(geno, "matrix")
fitM <- snpClust(geno, h, "R.squared")
```
