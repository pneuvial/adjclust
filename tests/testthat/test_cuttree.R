...
test_that("'cuttree' must ignore 'h' when reversals are present.", {
  data("iris")
  ...
  clust1 <- cutree(fit, k = 4)
  expect_error(cutree(fit, h = 0.02))
  clust2 <- cutree(fit, k = 4, h = 50)
  expect_equal(clust1, clust2)
  
  clust3 <- cutree(fit2, k = 4)
  expect_equal(clust1, clust3)
  height4four <- (fit2$height[nrow(sim) - 3] + fit2$height[nrow(sim) - 4])/2
  clust4 <- cutree(fit2, h = height4four)
  expect_equal(clust1, clust4)
  clust5 <- cutree(fit2, k = 4, h = 50)
