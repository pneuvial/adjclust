...
test_that("Output of the modifySparse function satisfies the conditions :For all 1 <= i,j <=p, (s[i,i] == 1) and (s[i,j] <= 0.5*(s[i,i]+s[j,j])) )", {
  ...
  sb1 <- sb
  expect_true(adjclust:::sparseCondnCheck(sb1, p, h))
  res1 <- adjclust:::modifySparse(sb, p, h)
  expect_true(adjclust:::sparseCondnCheck(res1, p, h))
  ...
  sb3 <- sb
  sb3[2,1] <- 2.2
  expect_true(adjclust:::sparseCondnCheck(sb3, p, h))
  ...
  sb4[3,2] <- 2.5
  sb4[4,1] <- 2.2
  expect_false(adjclust:::sparseCondnCheck(sb4, p, h))
