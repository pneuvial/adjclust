...
In this short vignette we demonstrate the use of `lociclust` function in the `adjclust` package.`lociclust` performs adjacency-constrained hierarchical clustering of genomic regions or loci.`lociclust` is a specialized version of `adjClustBand_heap` function designed specially to perform clustering of genomic regions or loci.This function implements the algorithm same as `adjClustBand_heap` that is described in the third chapter of [2]. It is an extension of the algorithm described in [3].  
...
## Using lociclust function for various input classes
...
Now that we have a text file `tf`, we are ready to pass it as input to the function `lociclust`
